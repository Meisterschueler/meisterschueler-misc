...
p = re.compile("(\\\\hyperref\\[)(\\w+?)(\\]\\{[\\w\\s^\\}]*?([IV]+\\.)?[0-9]+?[\\w\\s^\\}]+?\\})", re.DOTALL)
s = re.sub(p, lambda m: "\\hyperref[%s]{\\autoref{%s}}"
    % (m.group(2), m.group(2)), s)

p = re.compile("(\\\\hyperref\\[)(\\w+?[0-9]+)(\\]\\{[\\w\\s^\\}]*?([IV]+\\.)?[0-9]+?\\})", re.DOTALL)
s = re.sub(p, lambda m: "\\hyperref[%s]{\\autoref{%s}}"
    % (m.group(2), m.group(2)), s)

p = re.compile("(\\\\hyperref\\[)(\\w+?)(\\]\\{)("
    "(?!\\\\)(?!s\\.)(?!\\(s\\.)(?!II)(?!\\#)"
    "(?!Abschn)(?!Kapit)(?!Quellen)(?!Inhaltsverzeichnis)"
    "(?!Eigeldinger)(?!Fink)(?!Gieseking)(?!Otto)(?!Sandor)(?!Prokop)(?!Richmann)(?!Whiteside).*?)(\\})", re.DOTALL)
s = re.sub(p, lambda m: "\\hyperref[%s]{%s}\\index{%s}"
    % (m.group(2), m.group(4), m.group(4)), s)
